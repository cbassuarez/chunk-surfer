...
export function makeLensCalibrationScene({
  ...
  scene = {
    ...
    render() {
      ...
      uiCenter(y + 2, 'LOADING MATERIALS', 'ui-amber');
      ...
      const phase = error ? 'LOADING FAILED' : completed >= total ? 'READY' : phases[st.state] || 'STARTING';
      uiCenter(y + 7, phase, error ? 'ui-danger' : 'ui-secondary');
      if (!error) {
        const current = Math.min(total, completed + (completed < total ? 1 : 0));
        uiCenter(y + 9, completed < total ? `TEXTURE ${current} OF ${total}` : 'TEXTURES READY', 'ui-secondary');
        uiCenter(y + 11, `${Math.round((completed / total) * 100)}%`, 'ui-secondary');
      } else {
        const message = String(error).slice(0, Math.max(12, cols - 8));
        uiCenter(y + 9, message, 'ui-danger');
        uiCenter(y + 12, `${selected === 0 ? '▸' : ' '} RETRY`, selected === 0 ? 'ui-primary' : 'ui-secondary');
