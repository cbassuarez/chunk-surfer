...
const SETTING_HELP = Object.freeze({
  ...
  access: {
    ...
    personalInterference: 'PERSONALIZED INTERFERENCE is off by default and runs only during authored battles.',
    personalSourcesSteam: 'USE STEAM NAME reads only your current persona name. It does not request a Steam ID or friends list.',
    personalSourcesOs: 'USE OS USERNAME is a separate fallback used only when Steam persona lookup is unavailable.',
    personalSourcesHost: 'USE COMPUTER NAME allows the local host label to appear later in the interference sequence.',
    personalSourcesMic: 'USE SELECTED MIC LABEL uses a device label only after ordinary microphone permission already exists. It never records audio for this feature.',
    personalVfdText: 'USE IN SYSTEM TEXT controls whether resolved identity can appear as inert on-screen text. Identity is never sent to speech.',
    personalIntensity: 'LOW uses text and title only. STANDARD adds bounded game-window effects. HOSTILE permits short hide and minimize beats.',
    erasePersonalizedData: 'ERASE PERSONALIZED DATA deletes the local masking key, all field-return folders, and masked case references.',
