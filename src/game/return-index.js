  ...
  return {
    ...
    render() {
      ...
      const body = drawMachinePanel(x, y, w, h, {
        ...
        footer: dossier
          ? `R REVEAL FIELD RETURN · ${deletePending ? 'DEL CONFIRM DELETE' : 'DEL DELETE FIELD RETURN'} · ESC CLOSE`
          : '↑↓ ENDING · ESC CLOSE',
        meter: false,
      });
      ...
      entries.slice(scroll, scroll + cap).forEach((entry, j) => {
        ...
        uiText(body.x + 5, body.y + 6 + j * 3, entry.seen ? 'SEEN' : entry.adjacent ? 'LEAD' : 'LOCKED', entry.seen ? 'ui-green' : entry.adjacent ? 'ui-blue' : 'ui-secondary');
      });
      ...
      uiText(dx, body.y + 10, `STATUS  ${entry.seen ? 'SEEN' : entry.adjacent ? 'LEAD FILED' : 'LOCKED'}`, entry.seen ? 'ui-green' : entry.adjacent ? 'ui-blue' : 'ui-secondary');
      if (dossier) {
