...
export const CHUNK_SURF_ROOMS = Object.freeze([
  {
    ...
    lines: [
      'CORRIDOR.LENGTH = CORRIDOR.LENGTH + STEP;',
      'WORK_ORDER[4417-C].PAGES += 1;',
      'if (operator.turns_back) hush(operator);',
      'THE CHAPEL IS STILL AHEAD. IT HAS BEEN AHEAD FOR TOO LONG.',
    ],
  ...
  },
  {
    ...
    lines: [
      'OBJECT: TUNING_FORK',
      'STATUS: ISSUED_TO_PREVIOUS_RECORDIST',
      'OWNER: NOT RETURNED',
      'A = 440',
      'A != 440',
    ],
    tunedLines: [
      ...
      'STATUS: ISSUED_TO_PREVIOUS_RECORDIST',
      'OWNER: STILL WALKING',
      'A = THE ROOM BEFORE THE ROOM',
      'FALSE TEXT VIBRATES FIRST.',
    ],
  ...
  },
  {
    ...
    lines: [
      'const recordist = file("4417-C.previous");',
      'recordist.delivered = 4;',
      'recordist.returned = false;',
      'recordist.body = "available";',
    ],
    tunedLines: [
      'const recordist = file("4417-C.previous");',
      'recordist.delivered = 4;',
      'recordist.returned = false;',
      'recordist.body = surfer.form;',
      'HE DID NOT VANISH. HE AGREED.',
    ],
  ...
  },
  {
    ...
    tunedLines: [
      'student.practice(hours++);',
      'student.listen(music);',
      'student = music INSIDE THE FILES;',
      'music.needs(body);',
      'THE SURFER WAS NOT BORN IN THE BUILDING. IT WAS TRAINED THERE.',
      'THE MACHINE BOASTED NO LANGUAGE. THEN IT LEARNED TO ASK.',
    ],
  ...
  },
  {
    ...
    lines: [
      'DELIVER: five clean minutes;',
      'ACCEPTANCE: no breath, no handling noise;',
      'IF fifth == missing: send another body;',
      'ACCOUNT: open;',
    ],
    tunedLines: [
      'DELIVER: five clean minutes;',
      ...
      'IF fifth == missing: send another body;',
      'ACCOUNT: FEEDING;',
      'THE CONTRACT WAS NOT A TRAP. IT WAS A MOUTH SOMEONE KEPT SIGNING.',
    ],
  ...
  },
  {
    ...
    lines: [
      'surfer.form = previousRecordist.body;',
      'previousRecordist.signal = stillWalking;',
      'operator.body = nextCandidate;',
      'if (operator.agrees) return surfer;',
    ],
    tunedLines: [
      'surfer.form = previousRecordist.body;',
      'previousRecordist.signal = stillWalking;',
      'operator.body = nextCandidate;',
      'if (operator.refuses) body can be returned;',
      'THE FIFTH ROOM IS NOT THE CHAPEL. THE FIFTH ROOM IS CONSENT.',
      'SILENCE WAS THE VIRTUE IT USED TO MAKE MEN AGREEABLE.',
    ],
    inspect: 'The room is full of outlines. None of them are empty.',
    tune: 'Every outline turns toward the fork except the one that is trying not to.',
