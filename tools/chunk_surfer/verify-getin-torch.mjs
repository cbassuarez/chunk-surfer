...
for (const on of [false, true]) {
  const t = await page.evaluate((v) => window.__probe.setTorch(v), on);
  await new Promise((r) => setTimeout(r, 3000));
  await recover(4);
  console.log(`\ntorch ${on ? 'ON ' : 'OFF'} ${JSON.stringify(t)}`);
  console.log('  ', JSON.stringify(await read()));
  await page.screenshot({ path: `artifacts/getin-torch-${on ? 'on' : 'off'}.png` });
}
