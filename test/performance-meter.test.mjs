...
test('performance meter estimates fps from frame deltas', () => {
  ...
  assert.ok(snapshot.p95FrameMs > 16 && snapshot.p95FrameMs < 17);
...
test('performance meter can reset', () => {
  ...
  assert.deepEqual(meter.snapshot(), { fps: null, frameMs: null, p95FrameMs: null, lastFrameMs: null, maxFrameMs: null, spikesAbove50: 0, samples: 0 });
});
