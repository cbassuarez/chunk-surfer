...
assert.deepEqual(SOURCE_GET_IN_BOUNDS, { minX: 58, maxX: 72, minY: 4, maxY: 14 });
...
assert.deepEqual(SOURCE_GET_IN_DOOR_IDS,['dock-grey-exterior']);
...
assert.equal(doorPlacements.filter((entry)=>entry.id.includes('door-leaf')).length,2,'the exact grey goods pair is reconstructed closed');

const openingPlane = sourceLandingLocalFromAuthored({ x: SOURCE_GET_IN_BOUNDS.maxX + 1, y: 9.5 });
assert.equal(sourceLandingCellAt(openingPlane.x, openingPlane.y).opening, true, 'the forward wall is literally absent');
...
const openingWorld=world(SOURCE_LANDING_OPENING_LOCAL);
assert.ok(runtime.geometry.cellAt(openingWorld.x,openingWorld.y),'the removed forward wall still opens onto Source');
assert.ok(runtime.geometry.renderCellAt(openingWorld.x,openingWorld.y),'the opening has matching rendered ground');
...
// wrap around the room behind its authored forward edge.
{
  ...
  for(let index=0;index<queue.length;index+=1){
    ...
    const landing=sourceLandingCellAt(local.x,local.y);
